...
function calcularResultado() {
        ...
        if (impactoRespuestas[nombrePregunta]) {
            for (const [categoria, carrerasImpacto] of Object.entries(impactoRespuestas[nombrePregunta])) {
                for (const [carrera, impacto] of Object.entries(carrerasImpacto)) {
                    if (valorRespuesta === 1) {
                        afinidad[carrera][categoria] += impacto; // Incrementa significativamente
                    } else if (valorRespuesta === 2) {
                        afinidad[carrera][categoria] += impacto / 2; // Incrementa ligeramente
                    }
                }
            }
        }
    ...
    const carrerasOrdenadas = Object.keys(afinidad).sort((a, b) => {
        const totalAfinidadA = afinidad[a].intereses + afinidad[a].habilidades + afinidad[a].personalidad;
        const totalAfinidadB = afinidad[b].intereses + afinidad[b].habilidades + afinidad[b].personalidad;
...
function mostrarGrafico(afinidad, carrerasTop3) {
    ...
    const labels = carrerasTop3;
    const data = {
        labels: labels,
        datasets: [{
            label: 'Afinidad',
            data: labels.map(carrera => 
                afinidad[carrera].intereses + afinidad[carrera].habilidades + afinidad[carrera].personalidad),
