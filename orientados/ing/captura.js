...
function generarPDF(carreras, afinidad) {
    // Asegúrate de que jsPDF esté correctamente referenciado
    ...
    const carrera = localStorage.getItem("carrerausuario");

    // Título del PDF
    doc.setFontSize(18);
    doc.text("Resultados del Test de Orientación Vocacional", 10, 10);
    ...
    y += 10;
    doc.text(`Carrera: ${carrera}`, 10, y);
    
    
    // Añadir un espacio antes de las respuestas
    y += 10;

    // Respuestas seleccionadas (usando respuestasCapturadas, si lo tienes)
    doc.text("Respuestas del Test:", 10, y);
    y += 10;
    for (let pregunta in respuestasCapturadas) {
        doc.text(`${pregunta}: ${respuestasCapturadas[pregunta].descripcion} (Valor: ${respuestasCapturadas[pregunta].valor})`, 10, y);
        y += 10;
    }

    // Añadir un espacio antes de las carreras recomendadas
    y += 10;

    // Carreras recomendadas
    doc.text("Carreras Recomendadas:", 10, y);
    y += 10;
    carreras.forEach(carrera => {
        doc.text(`${carrera} - Afinidad: ${afinidad[carrera]}`, 10, y);
        y += 10;
    });
